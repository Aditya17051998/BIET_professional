import React, { Component } from 'react';
import {connect} from 'react-redux';
import { fetchDashboard } from '../actions/UserDashBoard';

class DashBoard_page extends Component {
    constructor(props){
        super(props);
        this.state={
            allUser:true,
        }
    }
    handleSearch=(year)=>{
        this.setState({
            allUser:false,
        })
        this.props.dispatch(fetchDashboard(year));
        
    }
    handleAllSearch=(e)=>{
        this.setState({
            allUser:true,
        })
    }
    
    render() {
        const {userslist,userList}=this.props;
        var users=[];
        console.log('userlistdashboar' ,userslist);
        if(this.state.allUser){
            users=userList;
        }
        else{
            users=userslist;
        }
        return (
            <div style={{height:"72vh",overflowY:"scroll"}}>
                <div className="homedashboardhead"><h1>User Dashboard</h1></div>
                <div className="sortTab" style={{display:"flex",justifyContent:"space-around",height:"10vh",width:"70%",marginLeft:"15%",background:"grey",color:"white"}}>
                    <span onClick={this.handleAllSearch} style={{marginTop:"2vh",fontSize:"30px",padding:"10px",borderRadius:"30px 30px 0px 0px"}}>Show All</span>
                    <span onClick={()=>this.handleSearch(1)} style={{marginTop:"2vh",fontSize:"30px",padding:"10px",borderRadius:"30px 30px 0px 0px"}}>1st Year</span>
                    <span onClick={()=>this.handleSearch(2)} style={{marginTop:"2vh",fontSize:"30px",padding:"10px",borderRadius:"30px 30px 0px 0px"}}>2nd Year</span>
                    <span onClick={()=>this.handleSearch(3)} style={{marginTop:"2vh",fontSize:"30px",padding:"10px",borderRadius:"30px 30px 0px 0px"}}>3rd Year</span>
                    <span onClick={()=>this.handleSearch(4)} style={{marginTop:"2vh",fontSize:"30px",padding:"10px",borderRadius:"30px 30px 0px 0px"}}>4th Year</span>
                </div>

                { users.map(user=>(
                   <div id="homedashboard">

                   <div class="homedashboard">
                       
                        {user.avatar?<img src={`https://biet-backend.herokuapp.com/${user.avatar}`} style={{height:"30px",width:"30px",borderRadius:"50%",marginLeft:"10px"}}/>:<img style={{height:"30px",borderRadius:"50%",marginLeft:"10px"}} src="data:image/png;base64," />}
                        <div style={{marginLeft:"5px",width:"17vw"}}> <b>{user.name}</b></div>
                        <div>3rd year</div>
                        <div style={{flexGrow:"3",marginLeft:"35px"}}>{user.email}</div>
                        <div style={{marginRight:"10px"}}><button>80091919936</button></div>
                    </div>
 

                </div>
                                
                ))}
                

            </div>
        );
    }
}

const mapStateToProps = (state) => ({  
    userslist:state.dashboard,
    userList:state.userslist,
  });
  
  export default connect(mapStateToProps)(DashBoard_page);